'use client';
// components/Grid.tsx — stats header, filter bar, and the dex slots (3 styles).
import { memo, useState, type CSSProperties } from 'react';
import { POKE_TYPES, GENERATIONS, SPRITE } from '@/lib/pokedata';
import { Icon, STATUS, StatusPill, typeGradient } from '@/components/ui';
import { formatEuro } from '@/lib/format';
import type { CardStyle, Entry, Mon, Stats, Status } from '@/lib/types';

export const DEX_BG: Record<CardStyle, { page: string; card: string; text: string; sub: string }> = {
...
export function slotIntrinsic(style: CardStyle, cols: number): number {
  const three = cols >= 3;
...
// ── stats header ─────────────────────────────────────────────────────────────
export function StatsHeader({ stats, style, onOpenSettings }: { stats: Stats; style: CardStyle; onOpenSettings: () => void }) {
  ...
  return (
    <div style={{ padding: '4px 18px 12px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 30, fontWeight: 800, letterSpacing: -0.5, lineHeight: 1, color: retro ? '#fff' : '#13151a', fontFamily: retro ? '"DM Mono", ui-monospace, monospace' : 'inherit' }}>Pokédex</div>
          <div style={{ fontSize: 12.5, color: sub, marginTop: 4, fontWeight: 500 }}>Card collection</div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 26, fontWeight: 800, color: accent, lineHeight: 1, fontVariantNumeric: 'tabular-nums', fontFamily: retro ? '"DM Mono", ui-monospace, monospace' : 'inherit' }}>
              {stats.owned}
              <span style={{ fontSize: 15, color: retro ? 'rgba(255,255,255,.55)' : '#b3b8c0', fontWeight: 700 }}> / {stats.total}</span>
            </div>
            <div style={{ fontSize: 11.5, color: sub, fontWeight: 600, marginTop: 2 }}>{pct}% collected</div>
          </div>
          <button onClick={onOpenSettings} aria-label="Settings" style={gearStyle}>
            <Icon.cog s={20} />
          </button>
        </div>
      </div>
...
function ValueChip({ retro, color, label, value }: { retro: boolean; color: string; label: string; value: number }) {
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '7px 12px', borderRadius: 12, background: retro ? 'rgba(0,0,0,.28)' : '#fff', boxShadow: retro ? 'none' : '0 1px 3px rgba(0,0,0,.06)' }}>
...
export function FilterBar({
  ...
  const statusOpts: [string, string, number][] = [
    ['all', 'All', counts.all],
    ...
    <div style={{ padding: '0 14px 8px', display: 'flex', flexDirection: 'column', gap: 9 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '0 12px', height: 40, borderRadius: 12, ...field }}>
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search Pokémon…" style={{ flex: 1, border: 0, outline: 0, background: 'transparent', font: 'inherit', fontSize: 15, color: 'inherit', minWidth: 0 }} />
          {q && (
            <button onClick={() => setQ('')} style={{ border: 0, background: 'transparent', color: retro ? 'rgba(255,255,255,.5)' : '#aab0b8', cursor: 'pointer', display: 'flex', padding: 2 }}>
              <Icon.close s={16} />
            </button>
          )}
        </div>
        ...
        {statusOpts.map(([key, label, n]) => {
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 11, fontWeight: 600, opacity: 0.82, marginTop: 2, fontVariantNumeric: 'tabular-nums' }}>{n}</div>
            </button>
          );
        })}
      </div>
      ...
      {showTypes && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, paddingTop: 2 }}>
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  color: '#fff',
                  background: POKE_TYPES[t].c,
                  opacity: active ? 1 : retro ? 0.4 : 0.42,
                  outline: active ? '2px solid rgba(255,255,255,.9)' : 'none',
                  outlineOffset: -2,
          ...
          {types.length > 0 && (
            <button onClick={() => setTypes([])} style={{ padding: '5px 11px', borderRadius: 999, border: 0, cursor: 'pointer', fontSize: 11.5, fontWeight: 700, background: retro ? 'rgba(255,255,255,.15)' : '#eceef2', color: retro ? '#fff' : '#5b6068' }}>
              Clear
            </button>
          )}
        </div>
...
export const SlotCard = memo(function SlotCard({
  ...
  const cv: CSSProperties = { contentVisibility: 'auto', containIntrinsicSize: `0 ${cvHeight}px` };
  ...
  const cardImg = showCard && card ? (
    <img src={card.img} alt={card.name} loading="lazy" decoding="async" style={{ width: '100%', height: '100%', objectFit: 'contain', filter: status === 'want' ? 'saturate(.92)' : 'none' }} />
  ) : null;

  // ----- RETRO ---------------------------------------------------------------
  if (style === 'retro') {
    return (
      <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', textAlign: 'left', cursor: 'pointer', border: '1px solid rgba(124,252,155,.22)', borderRadius: 8, padding: 7, gap: 6, background: 'rgba(0,0,0,.28)', color: '#d7ffe2', fontFamily: '"DM Mono", ui-monospace, monospace', minWidth: 0, ...cv }}>
        ...
        <div style={{ aspectRatio: '63/88', borderRadius: 5, overflow: 'hidden', background: 'rgba(124,252,155,.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', boxShadow: 'inset 0 0 0 1px rgba(124,252,155,.18)' }}>
          {showCard ? cardImg : (
            <div style={{ textAlign: 'center', width: '100%' }}>
              <img src={SPRITE.pixel(mon.id)} alt="" loading="lazy" style={{ width: '64%', imageRendering: 'pixelated', filter: 'brightness(0) invert(1) opacity(.25) drop-shadow(0 0 1px #7CFC9B)' }} />
  ...
  // ----- PLAYFUL -------------------------------------------------------------
  if (style === 'playful') {
    const bg = typeColors ? `linear-gradient(160deg, ${grad[0]}, ${grad[1]})` : 'linear-gradient(160deg,#3a3550,#221d38)';
    return (
      <button onClick={onClick} style={{ position: 'relative', display: 'flex', flexDirection: 'column', textAlign: 'left', cursor: 'pointer', border: 0, borderRadius: 20, padding: 11, gap: 8, background: bg, color: '#fff', boxShadow: '0 8px 18px rgba(0,0,0,.22)', overflow: 'hidden', minWidth: 0, ...cv }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 1 }}>
          <span style={{ fontSize: 12, fontWeight: 800, opacity: 0.85 }}>{numStr}</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, background: 'rgba(255,255,255,.22)', padding: '3px 8px', borderRadius: 999, fontSize: 10.5, fontWeight: 800 }}>
            {status === 'owned' && <Icon.check s={11} />}
            {status === 'want' && <Icon.bookmark s={10} />}
            {st.label}
          </span>
        </div>
        <div style={{ aspectRatio: '63/88', borderRadius: 12, overflow: 'hidden', background: 'rgba(255,255,255,.16)', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1, backdropFilter: 'blur(2px)' }}>
          {showCard ? cardImg : (
            <div style={{ textAlign: 'center' }}>
              <img src={SPRITE.home(mon.id)} alt="" loading="lazy" style={{ width: '74%', filter: 'brightness(0) invert(1) opacity(.32)' }} />
              <div style={{ fontSize: 14, fontWeight: 900, letterSpacing: 1, opacity: 0.6, marginTop: -6 }}>—</div>
            </div>
          )}
        </div>
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginTop: 4 }}>
            {mon.types.map((t) => (
              <span key={t} style={{ fontSize: 9.5, fontWeight: 800, background: 'rgba(0,0,0,.22)', padding: '2px 7px', borderRadius: 999 }}>{POKE_TYPES[t].name}</span>
            ))}
          </div>
        </div>
      </button>
  ...
  return (
    <button onClick={onClick} style={{ display: 'flex', flexDirection: 'column', textAlign: 'left', cursor: 'pointer', border: 0, borderRadius: 16, padding: 9, gap: 8, background: '#fff', color: '#1b1d21', boxShadow: '0 1px 2px rgba(20,30,50,.05), 0 6px 16px rgba(20,30,50,.06)', borderTop: typeColors ? `3px solid ${accent}` : '3px solid transparent', minWidth: 0, ...cv }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 11.5, fontWeight: 800, color: '#b3b8c0', fontVariantNumeric: 'tabular-nums' }}>{numStr}</span>
        <StatusPill status={status} />
      </div>
      <div style={{ aspectRatio: '63/88', borderRadius: 10, overflow: 'hidden', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', background: showCard ? '#f4f5f8' : typeColors ? `linear-gradient(160deg, ${grad[0]}22, ${grad[1]}14)` : '#f4f5f8', boxShadow: showCard ? 'none' : 'inset 0 0 0 2px rgba(0,0,0,.04)' }}>
        {showCard ? cardImg : (
      ...
      <div>
        ...
        <div style={{ fontSize: 11, color: '#9aa0a6', marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {showCard && card ? `${card.setName} · ${card.number}` : mon.types.map((t) => POKE_TYPES[t].name).join(' / ')}
        </div>
      </div>
    </button>
  );
