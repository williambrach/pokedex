'use client';
// components/IOSFrame.tsx — responsive shell.
// Large screens: a polished iOS device mockup. Phones: edge-to-edge full-screen.
...
export function useViewport(): boolean {
  const get = () =>
    typeof window !== 'undefined' && window.innerWidth >= 760 && window.innerHeight >= 720;
...
// Scale the 402×874 mockup to fit the viewport (sets the --scale CSS var that
// .device-wrap consumes). Replaces the inline <script> from the old index.html.
export function useDeviceScale() {
  useEffect(() => {
    const fit = () => {
      ...
      document.documentElement.style.setProperty('--scale', String(Math.max(s, 0.2)));
    };
    fit();
