// app.jsx — Pokédex app root: data loading, filtering, grid, detail, settings.

function LoadScreen({ progress, error, onRetry }) {
  return (
...
function App() {
  ...
  const [mons, setMons] = React.useState(null);
  const [progress, setProgress] = React.useState(0);
  const [loadErr, setLoadErr] = React.useState(false);
  const { coll, update, reset } = useCollection();
  ...
  const load = React.useCallback(() => {
    setLoadErr(false); setProgress(0);
    loadPokedex(setProgress).then(setMons).catch(() => setLoadErr(true));
  }, []);
  React.useEffect(() => { load(); }, [load]);

  // filtering (everything except status) → counts → status filter → list
  const { list, counts, stats } = React.useMemo(() => {
    if (!mons) return { list: [], counts: { all: 0, owned: 0, want: 0, tba: 0 }, stats: { owned: 0, want: 0, value: 0, wishValue: 0, total: 0, missing: 0, pct: 0 } };
    ...
    base.forEach(m => { counts[statusOf(coll[m.id])]++; });
  ...
  const openMon = openId != null ? mons.find(m => m.id === openId) : null;

  const headerBg = settings.cardStyle === 'retro' ? 'linear-gradient(#b3261e,#8f1d17)'
    : settings.cardStyle === 'playful' ? 'linear-gradient(#221d38,#1a1530)'
    : 'linear-gradient(#fbfbfd,#eef0f4)';

  const safeVars = framed
    ? { '--safe-top': '50px', '--safe-bottom': '34px' }
    : { '--safe-top': 'calc(env(safe-area-inset-top, 0px) + 14px)', '--safe-bottom': 'env(safe-area-inset-bottom, 0px)' };

  const onReset = () => {
    if (window.confirm('Clear your whole collection? This removes every owned/wishlist mark and photo.')) {
      reset(); setShowSettings(false);
    }
  };
  ...
  const screen = (
    <div className="device-screen" style={{ position: framed ? 'absolute' : 'fixed', inset: 0, display: 'flex', flexDirection: 'column', background: bg.page, ...safeVars }}>
      {!mons ? (
        ...
        <>
          <div style={{ paddingTop: 'var(--safe-top)', flexShrink: 0, background: headerBg,
            borderBottom: settings.cardStyle === 'album' ? '1px solid #e2e5ea' : 'none',
            boxShadow: settings.cardStyle === 'album' ? '0 2px 12px rgba(0,0,0,.03)' : 'none', position: 'relative', zIndex: 2 }}>
            <StatsHeader stats={stats} style={settings.cardStyle} onOpenSettings={() => setShowSettings(true)} />
            <FilterBar q={q} setQ={setQ} status={statusF} setStatus={setStatusF}
              gen={gen} setGen={setGen} types={typeF} setTypes={setTypeF} counts={counts} style={settings.cardStyle} />
          </div>
          <div style={{ flex: 1, overflow: 'auto', WebkitOverflowScrolling: 'touch' }}>
            {list.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 30px', color: bg.sub, fontSize: 14.5, lineHeight: 1.5 }}>
              ...
              <div key={`grid-${cols}-${settings.cardStyle}`} style={{ display: 'grid', gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`, gap: cols >= 3 ? 8 : 11, padding: '12px 14px calc(var(--safe-bottom) + 30px)' }}>
                {list.map(m => (
                  <SlotCard key={m.id} mon={m} entry={coll[m.id]} status={statusOf(coll[m.id])}
                    style={settings.cardStyle} typeColors={settings.typeColors} cvHeight={cvHeight} onClick={() => setOpenId(m.id)} />
                ))}
              </div>
            )}
          </div>
      ...
      {openMon && (
        <DetailSheet mon={openMon} entry={coll[openMon.id]} status={statusOf(coll[openMon.id])}
          onUpdate={update} onClose={() => setOpenId(null)} />
      )}

      {showSettings && (
        <SettingsSheet settings={settings} setSetting={setSetting} onReset={onReset} onClose={() => setShowSettings(false)} />
      )}
    </div>
  );
